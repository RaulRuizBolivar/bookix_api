...
// const getVotes = book_club_id => {
// 	return executeQuery( 'select user_id , book_id from historial where book_club_id = ?', [ book_club_id ] )
// }
...
const getUsersVote = ( book_club_id, book_id ) => {
	return executeQuery( "select  h.user_id as id, (select username from users where id = h.user_id) as username, (select email from users where id = h.user_id) as email, (select name from users where id = h.user_id) as name, (select image from users where id = h.user_id) as image, (select count(book_id) + 1 as vote_weight from historial where user_id = h.user_id and action = 'read' and book_club_id = ?) as vote_weight from historial h inner join book_club bc on h.book_club_id = bc.id where h.date > bc.changed_phase and h.book_club_id = ? and action = 'vote' and book_id = ?;", [ book_club_id, book_club_id, book_id ] )
}

const getArrBooksVoted = book_club_id => {
	return executeQuery( "select h.book_id as id, (select title from books where id = h.book_id) as title, (select author from books where id = h.book_id) as author, (select num_pages from books where id = h.book_id) as num_pages, (select genre from genres where id = (select genre_id from book_club where id = ?)) as genre, (select synopsis from books where id = h.book_id) as synopsis, (select front_page from books where id = h.book_id) as front_page from historial h inner join book_club bc on h.book_club_id = bc.id where h.date > bc.changed_phase and h.book_club_id = ?  and action = 'vote' group by h.book_id;", [ book_club_id, book_club_id ] )
}
